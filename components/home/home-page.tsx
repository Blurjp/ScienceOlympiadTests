...
export default function HomePage({ initialTests, initialTopics }: HomePageProps) {
  ...
  const [availableTopics, setAvailableTopics] = useState<string[]>(
    initialTopics.length > 0 ? initialTopics : DEFAULT_TOPICS
  );
  ...
  // Revalidate server-provided data on mount (initial data comes from SSR)
  useEffect(() => {
    if (initialTests.length === 0) loadTests();
    if (initialTopics.length === 0) loadTopics();
    loadPdfSources();
  }, []);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-6xl px-4 py-8">
        ...
        {/* Ad after generators - only when real content is loaded (AdSense policy) */}
        ...
        {/* Ad before footer - only when real content is loaded (AdSense policy) */}
        {!isLoading && tests.length > 0 && process.env.NEXT_PUBLIC_GOOGLE_AD_SLOT_2 && (
